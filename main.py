...
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET", "nullprotocol-bot")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Bas User ID bhej do (jaise 7623390524) ya neeche buttons se start karo!",
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Help section bhai:\n\n"
        "• Direct User ID type kar do (sirf numbers, example: 7623390524)\n"
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "search":
        await query.message.reply_text("Bhai ab User ID bhej do (example: 7623390524) 😎")
...
async def search_userid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not userid.isdigit():
        await update.message.reply_text("❌ Galat input bhai! Sirf numbers daalo (jaise 7623390524)")
        return

    searching_msg = await update.message.reply_text(f"🔍 User ID {userid} check kar raha hoon... Ruk ja 2 sec!")
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            error_text = f"⚠️ Oops! Kuch gadbad ho gayi\nError: {str(e)}\nThodi der baad phir try karna bhai"
            error_text += "\n\nDeveloper: @Nullprotocol_X\nPowered by: NULL PROTOCOL"
            await searching_msg.edit_text(error_text, reply_markup=get_after_search_keyboard())

# ────────────────────────────────────────────────
# Keep-Alive Ping (Free Render plan ke liye - har 5 min ping)
# ────────────────────────────────────────────────
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=10) as resp:
                    print(f"✅ Keep-alive ping: {resp.status} - Server active rakha gaya")
        except Exception as e:
...
async def startup_event():
    ...
    webhook_url = f"https://{hostname}/{WEBHOOK_SECRET}" if hostname else f"https://tg2num1.onrender.com/{WEBHOOK_SECRET}"

    print(f"Webhook URL ban gaya: {webhook_url}")
    ...
    try:
        ...
        print(f"Webhook set error: {str(e)}")

    # Keep-alive start
    asyncio.create_task(keep_alive_ping())
    print("✅ Keep-alive ping shuru ho gaya (har 5 minute)")
...
# Local Testing Mode (Deploy ke time isko comment out kar dena)
if __name__ == "__main__":
    print("Local testing mode (Polling) chal raha hai...")
    application = Application.builder().token(TOKEN).build()
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("help", help_command))
    application.add_handler(CommandHandler("search", search_userid))
